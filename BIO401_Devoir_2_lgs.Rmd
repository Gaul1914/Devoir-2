...
$\gamma$ = $\epsilon\beta$ 
$\epsilon$ = taux de transfert entre proies et prédateurs ????
...
for (i in 1:100){
  res_b[i,1]<-runif(1,min=0,max=0.2)
  b<-res_b[i,1]
  parametre<-c(alpha=a,k=k,beta=b,gamma=g,delta=d)
  ProiepredRes <- ode(y = condIni, times = temps, func = proiepred,parms = parametre)
  cor_proie<-cor.test(ProiepredRes[,"R"],bact$bogustonia_proii)
...
for (i in 1:100){
  res_d[i,1]<-runif(1,min=0.2,max=1)
  d<-res_d[i,1]
  parametre<-c(alpha=a,k=k,beta=b,gamma=g,delta=d)
  ProiepredRes <- ode(y = condIni, times = temps, func = proiepred,parms = parametre)
  cor_proie<-cor.test(ProiepredRes[,"R"],bact$bogustonia_proii)
...
cor_R_exp<-cor_pred_exp$estimate
```
